            ...
            <table class="table datatable table-responsive-x">
                ...
                <tbody>
                    ...
                    <tr wire:key='{{$add_product->id}}'>
                        ...
                        <td class="d-flex">
                            <span><button type="button" wire:click='soldOut({{$add_product->id}})' wire:confirm='Are you sure you want to declare this product unavailable in stock ?' class="badge bg-danger border m-1">sold out</button></span>
                            <span><button type="button" wire:click='inStock({{$add_product->id}})' wire:confirm='Are you sure you want to declare this product available in stock ?' class="badge bg-info border m-1">in stock</button></span>
                            {{-- <button type="button" class="btn btn-sm btn-danger m-1">Sold out</button> --}}
                        </td>
                        <td>
                            <button type="button" wire:click='delete({{$add_product->id}})' wire:confirm='Are you sure you want to delete this product ?' class="btn btn-sm btn-danger m-1">
